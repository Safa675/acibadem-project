...
def compute_garch_health_vol(
    ...
    vol_data = [
        {"date": returns[i].date, "forecast_vol": round(forecast_vols[i], 2),
         "realized_vol": round(realized_vols[i], 2), "regime": regime}
...
def run_clinical_stress_test(
    ...
    es_val = float(np.mean(tail)) * 100 if len(tail) > 0 else float(np.percentile(arr, 5)) * 100
    scenarios = [
        {"name": "Worst Day (actual)", "shock_pct": round(float(arr.min()) * 100, 2),
         "health_impact_pct": round(float(arr.min()) * 100, 2),
         "expected_shortfall": round(es_val, 2)},
        {"name": "99th percentile adverse", "shock_pct": round(float(np.percentile(arr, 1)) * 100, 2),
         "health_impact_pct": round(float(np.percentile(arr, 1)) * 100, 2),
         "expected_shortfall": round(es_val, 2)},
        {"name": "2x historical worst", "shock_pct": round(float(arr.min()) * 200, 2),
...
class WalkForwardClinicalResult:
    splits: list[dict]  # [{split, train_start, train_end, test_start, test_end, train_sharpe, test_sharpe}]
...
def run_walk_forward_clinical(
    ...
    for s in range(splits):
        ...
        test_sharpe = np.mean(test) / max(np.std(test), 1e-8) * np.sqrt(252) if test else 0
        split_data.append({
            "split": s + 1,
            "train_start": returns[0].date, "train_end": returns[min(train_end - 1, n - 1)].date,
            "test_start": returns[min(train_end, n - 1)].date, "test_end": returns[min(test_end - 1, n - 1)].date,
            "train_sharpe": round(train_sharpe, 3), "test_sharpe": round(test_sharpe, 3),
        })
    avg_ts = np.mean([s["test_sharpe"] for s in split_data]) if split_data else 0
    return WalkForwardClinicalResult(
        splits=split_data, avg_test_sharpe=round(avg_ts, 3),
...
def compute_suffering_metrics(
    ...
    gains = vals[vals > 0]
    losses = vals[vals < 0]
    pf = float(gains.sum() / abs(losses.sum())) if len(losses) > 0 and losses.sum() != 0 else None
...
def compute_organ_risk_contribution(
    ...
    return {
        "contribution_pct": {k: round(v / total_var * 100, 2) for k, v in contribs.items()},
...
def compute_kelly_intervention(
    ...
    if len(wins) == 0 or len(losses) == 0:
        return InterventionSizing(
            ...
            recommendation="Patient consistently improving" if len(losses) == 0 else "Patient consistently declining — urgent intervention",
        )

    win_rate = len(wins) / len(vals) * 100
    wl_ratio = abs(np.mean(wins) / np.mean(losses))

    kelly_frac = max(0, (win_rate / 100 - (1 - win_rate / 100) / wl_ratio)) * fractional_kelly
...
def compute_rolling_health_metrics(
    ...
    for i in range(len(vals)):
        if i < window:
            results.append(HealthRollingPoint(date=dates[i], rolling_return=None,
                                              rolling_sharpe=None, rolling_volatility=None,
        ...
        results.append(HealthRollingPoint(
            date=dates[i],
