            ...
            [
                {
                    ...
                    "state_color": STATE_COLORS.get(p.state, "#808080")
                    if p.state
                    else "#808080",
                }
...
class PatientStateClassifier:
    ...
    def classify_snapshots(self, patient_id: str, snapshots) -> PatientRegimeResult:
    ...
    def classify_dataframe(
        ...
        return self.classify_series(
            patient_id, df["health_score"].tolist(), df["date"].tolist()
        )
...
def classify_all_patients(
    snapshots_by_patient: dict[str, list],
    config: RegimeConfig | None = None,
) -> dict[str, PatientRegimeResult]:
...
def compute_cohort_stats(results: dict[str, PatientRegimeResult]) -> pd.DataFrame:
        ...
        rows.append(
            {
                ...
                "pct_critical": durations.get("Critical", 0)
                / max(len(result.timeline), 1),
            }
        )
...
def _rolling_mean(arr: np.ndarray, window: int) -> list[float | None]:
    ...
    rm = s.rolling(window=window, min_periods=window).mean()
    return [None if pd.isna(v) else float(v) for v in rm]


def _rolling_std(arr: np.ndarray, window: int) -> list[float | None]:
    s = pd.Series(arr, dtype=float)
    # Need at least 3 points for meaningful std
    rs = s.rolling(window=window, min_periods=3).std(ddof=1)
    return [None if pd.isna(v) else float(v) for v in rs]


def _rolling_percentile_rank(
    vols: list[float | None], lookback: int
) -> list[float | None]:
    """Compute the percentile rank of each vol value within its lookback window."""
    result: list[float | None] = []
    clean_vols: list[float | None] = []
    for v in vols:
        if v is None:
            result.append(None)
        ...
        if len(historical) < 3:  # need >=3 for non-degenerate ranking
